...
describe("initializeMessagingTransport", () => {
	...
	const createMockDriver =
		(label: string): TransportDependencies["createWebRTC"] =>
	...
	const createMockWebSocketDriver =
		(
			...
			context: {
				...
				onReady?: (
					session: Awaited<ReturnType<typeof createPeerCryptoSession>>,
				) => void;
			} = {},
	...
	it("connects using the websocket mode and relays messages", async () => {
		...
		let remoteSession: Awaited<
			ReturnType<typeof createPeerCryptoSession>
		> | null = null;
		...
		const dependencies: TransportDependencies = {
			pushEndpoint: (options) =>
				typeof options?.roomId === "string"
					...
					: undefined,
			EventSourceConstructor: MockEventSource as unknown as new (
				url: string,
			) => EventSource,
		};
	...
	it("recovers push connections after transient errors", async () => {
		...
		expect(
			states.filter((state) => state === "connected").length,
	...
	it("clears stale connections when the driver closes and allows reconnect", async () => {
		...
		const driverContext: {
			emitState: ((state: TransportState) => void) | null;
		} = {
			emitState: null,
		};
		...
		const handle = controller.transport as TransportHandle;
		const unsubscribe = handle.onStateChange((state) =>
		...
		await expect(handle.send("after-close")).rejects.toThrow(
	...
	it("refreshes the mode using stored preferences", async () => {
		const dependencies: TransportDependencies = {
			...
			udpConnector: {
				async join({ signal } = {}) {
					...
					let controller: ReadableStreamDefaultController<TransportMessage> | null =
						null;
		...
		const storage = (
			globalThis.window.localStorage as {
				setItem: (key: string, value: string) => void;
			}
		).setItem.bind(globalThis.window.localStorage);
	...
	it("retries handshake frames when the transport send rejects during bootstrap", async () => {
		const signalingController = createPeerSignalingController();
		signalingController.setRole("host");
		const manualDependencies = signalingController.createDependencies();
		...
		await new Promise<void>((resolve, reject) => {
			...
			const verify = () => {
				if (
					handshakeFrames.length > 0 &&
					currentHandle?.state === "connected"
				) {
					clearTimeout(timeout);
					resolve();
					return;
	...
	it("keeps sending handshake frames until the peer responds even after many retries", async () => {
	...
	it("retains negotiated tokens on disconnect and avoids republishing until reset", async () => {
		...
		expect(signalingController.getSnapshot().sessionId).not.toBe(
	...
	it("resends persisted offer handshakes after hydration", async () => {
		...
		const teardown = () => {
			(
				globalThis.window.localStorage as { storage: Map<string, string> }
		...
		try {
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("handshake not captured")),
					500,
				);
				const verify = () => {
			...
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("handshake not resent")),
					500,
				);
				const verify = () => {
	...
	it("continues resending persisted offers after hydration until a remote answer arrives", async () => {
		...
		await new Promise<void>((resolve, reject) => {
			const timeout = setTimeout(
				() => reject(new Error("offer not persisted")),
				500,
			);
			const verify = () => {
		...
		try {
			...
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("offer did not retry")),
					500,
				);
				const verify = () => {
	...
	it("resends persisted answer handshakes after hydration", async () => {
		...
		try {
			...
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("answer handshake not captured")),
					500,
				);
				const verify = () => {
					if (capturedHandshakes.length > 0) {
			...
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("answer handshake not resent")),
					500,
				);
				const verify = () => {
					if (resentHandshakes.length > 0) {
	...
	it("continues resending persisted answers after hydration until the connection is established", async () => {
		...
		try {
			...
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("answer not persisted")),
					500,
				);
				const verify = () => {
		...
		try {
			...
			await new Promise<void>((resolve, reject) => {
				const timeout = setTimeout(
					() => reject(new Error("answer did not retry")),
					500,
				);
				const verify = () => {
					if (resentHandshakes.length >= 2) {
	...
	it("applies offer handshakes delivered via broadcast channel", async () => {
		const hostController = createPeerSignalingController(
			"broadcast-offer-host",
		);
		hostController.setRole("host");
		const guestController = createPeerSignalingController(
			"broadcast-offer-guest",
		);
		...
		const waitForInvite = new Promise<void>((resolve, reject) => {
			const timeout = setTimeout(
				() => reject(new Error("Offer handshake not applied")),
				500,
			);
	...
	it("applies answer handshakes delivered via broadcast channel", async () => {
		const hostController = createPeerSignalingController(
			"broadcast-answer-host",
		);
		hostController.setRole("host");
		const guestController = createPeerSignalingController(
			"broadcast-answer-guest",
		);
		guestController.setRole("guest");
		...
		const waitForAnswer = new Promise<void>((resolve, reject) => {
			const timeout = setTimeout(
				() => reject(new Error("Answer handshake not applied")),
				500,
			);
