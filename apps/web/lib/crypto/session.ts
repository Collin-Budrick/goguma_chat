...
export type PeerCryptoSession = {
	...
	attachTransmitter: (
		transmit: (payload: TransportMessage) => Promise<void>,
	) => void;
...
const dbPromiseRef: { current: Promise<IDBDatabase | null> | null } = {
	current: null,
};
...
const loadIdentityRecord = async (): Promise<IdentityStoreRecord | null> => {
	...
	if (!db) {
		return (
			(inMemoryStores.keys.get(IDENTITY_KEY) as
				| IdentityStoreRecord
				| undefined) ?? null
		);
	}
	...
	return new Promise<IdentityStoreRecord | null>((resolve, reject) => {
		...
		request.onerror = () =>
			reject(request.error ?? new Error("Failed to load identity"));
		request.onsuccess = () =>
			resolve((request.result as IdentityStoreRecord | undefined) ?? null);
	});
};
...
const loadSessionRecord = async (
	sessionId: string,
): Promise<SessionStoreRecord | null> => {
	const db = await getDatabaseOnce();
	if (!db) {
		return (
			(inMemoryStores.sessions.get(sessionId) as
				| SessionStoreRecord
				| undefined) ?? null
		);
	}
	...
	return new Promise<SessionStoreRecord | null>((resolve, reject) => {
		...
		request.onsuccess = () =>
			resolve((request.result as SessionStoreRecord | undefined) ?? null);
	});
};
...
const isHandshakeEnvelope = (
	value: CryptoEnvelope,
): value is EnvelopeHandshake => value.kind === "handshake";
...
class PeerCryptoSessionImpl implements PeerCryptoSession {
	...
	private transmit: ((payload: TransportMessage) => Promise<void>) | null =
		null;
	...
	private async performEncryptedSend(payload: TransportMessage) {
		...
		try {
			...
			const envelope: EnvelopeData = {
				...
				rotation:
					this.remoteRotation && this.remoteRotation > this.localRotation
						? this.remoteRotation
						: this.localRotation,
	...
	private async persistTrustState() {
		...
		if (this.remotePublicKey) {
			const exportedRemoteKey = await crypto.subtle.exportKey(
				"raw",
				this.remotePublicKey,
			);
...
export const getPeerTrustState = async (
	sessionId: string,
): Promise<PeerTrustState> => {
	const identity = await getIdentityKeyPair();
