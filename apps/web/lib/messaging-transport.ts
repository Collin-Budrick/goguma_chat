...
export type TransportMessage = string | ArrayBuffer | ArrayBufferView | Blob;
...
const resolvePeerIceServers = (): RTCIceServer[] => {
	...
	const fromNavigator =
		typeof navigator !== "undefined"
			? (navigator as unknown as { gogumaIceServers?: unknown })
					.gogumaIceServers
			: undefined;
...
export type TransportDependencies = {
	...
	webTransportEndpoint?:
		| string
		| ((options?: TransportConnectOptions) => string);
...
class PeerNegotiationCancelledError extends Error {
	constructor(
		message = "Peer signaling negotiation cancelled before completion",
	) {
		super(message);
...
const isNegotiationCancelledError = (
	value: unknown,
): value is PeerNegotiationCancelledError =>
	value instanceof PeerNegotiationCancelledError;
	...
	try {
		...
		const message =
			error instanceof Error
				? error.message
				: "Failed to parse peer signaling token";
		throw new Error(message);
	}
};

const loadPersistentPeerState = (
	storageKey: string,
): PersistentPeerState | null => {
	...
	try {
		...
		const awaitingAnswer =
			typeof parsed.awaitingAnswer === "boolean"
				? parsed.awaitingAnswer
				: false;
...
export const createPeerSignalingController = (
	...
	let applyExternalHandshakeSignal:
		| ((signal: HandshakeSignal) => boolean)
		| null = null;
	...
	const broadcastHandshakeSignal = (
		handshake: PeerHandshakeFrame["handshake"],
	) => {
		if (typeof window === "undefined") {
			return;
	...
	const hasHandshakeExpired = (entry: HandshakeEntry) =>
		now() >= getHandshakeExpiration(entry);
	...
	const queueHandshake = (
		...
		let createdAt =
			typeof createdAtOverride === "number" ? createdAtOverride : now();
		if (typeof createdAtOverride !== "number") {
			try {
	...
	const hydratePersistentState = () => {
		...
		const hydratedSnapshot = snapshotFromPersistentState(
			pendingPersistentState,
		);
	...
	const handleOfferCreated = (
		...
		const token = serializePeerToken({
			...
			sessionId: currentState.sessionId,
			roomId:
				typeof connectOptions?.roomId === "string"
					? connectOptions.roomId
					: undefined,
	...
	const createDependencies = (): TransportDependencies => {
		const createManualWebRTC: NonNullable<
		...
		> = async (startOptions) => {
			...
			const attachChannel = (channel: RTCDataChannel) => {
				...
				channel.addEventListener("error", (event) => {
					...
					emitError(
						normalize(detail ?? new Error("WebRTC data channel error")),
					);
					emitState("error");
				});
			...
			if (role === "guest") {
				...
				if (offerPayload.kind !== "offer") {
					throw new TransportUnavailableError(
						"Remote invite token is not an offer",
					);
				}

				await peer.setRemoteDescription(offerPayload.description);

				const channelPromise = new Promise<RTCDataChannel>(
					(resolve, reject) => {
						const handleAbort = () => {
		...
		if (typeof BroadcastChannel !== "undefined") {
			try {
				...
				handshakeChannel.addEventListener("message", (event) => {
					enqueueExternalHandshake(
						(event as MessageEvent<HandshakeSignal>).data,
					);
				});
			} catch (error) {
...
const getGlobalUDPConnector = (): UDPConnector | undefined => {
	...
	const fromWindow = (globalScope as { __gogumaUDP?: UDPConnector })
		.__gogumaUDP;
	...
	const datagram = (navigator as unknown as { datagram?: UDPConnector })
		?.datagram;
...
const createUDPDriver = (
	...
	async start({ signal, options, emitMessage, emitError, emitState }) {
		...
		if (!connector) {
			throw new TransportUnavailableError(
				"UDP datagram connector is unavailable",
			);
		}
		...
		if (typeof session.addEventListener === "function") {
			...
			cleanupCallbacks.push(() =>
				session.removeEventListener?.("statechange", stateListener),
			);
			...
			cleanupCallbacks.push(() =>
				session.removeEventListener?.("message", messageListener),
			);
			...
			cleanupCallbacks.push(() =>
				session.removeEventListener?.("error", errorListener),
			);
		} else {
		...
		if (session.readable?.getReader) {
			pumpPromise = pumpReadableStream(
				session.readable,
				emitMessage,
				signal,
			).catch((error) => {
...
const defaultCreateWebRTC = async (
	...
	const { signal, emitMessage, emitError, emitState, dependencies, options } =
		startOptions;
	...
	const channelLabel =
		typeof options?.roomId === "string" ? options.roomId : "messaging";
	...
	if (!negotiate) {
		peer.close();
		throw new TransportUnavailableError(
			"Missing WebRTC signaling implementation",
		);
	}
	...
	const knownRelayFingerprints = new Set<string>(
		(dependencies.signaling?.iceServers ?? []).map((server) =>
		...
		try {
			...
			await Promise.all(
				removals.map((candidate) =>
					removeIceCandidate(candidate).catch(() => undefined),
				),
			);
		} catch (error) {
	...
	await new Promise<void>((resolve, reject) => {
		...
		if (channel.addEventListener) {
			...
			channel.onerror = handleError as (
				this: RTCDataChannel,
				ev: Event,
			) => void;
		}
...
const defaultCreateWebTransport = async (
	startOptions: DriverStartOptions & {
		dependencies: TransportDependencies;
		endpoint?: string;
	},
): Promise<DriverConnection> => {
	...
	if (!endpointCandidate) {
		throw new TransportUnavailableError(
			"WebTransport endpoint is not configured",
		);
	}
	...
	const textEncoderFactory =
		dependencies.textEncoder ?? (() => new TextEncoder());

	signal.addEventListener(
		"abort",
		() => {
			transport
				.close({ closeCode: 0, reason: "aborted" })
				.catch(() => undefined);
		},
		{ once: true },
	);
...
const defaultCreateWebSocket = async (
	...
	const socket: WebSocketLike & { readyState: number } = WebSocketCtor
		...
		: ({
				...
				send() {
					throw new TransportUnavailableError(
						"WebSocket constructor is unavailable",
					);
				},
				addEventListener() {},
				removeEventListener() {},
	...
	const handleMessage = (
		event: MessageEvent<TransportMessage> | { data: TransportMessage },
	) => {
...
const defaultCreatePush = async (
	...
	if (!endpointCandidate) {
		const error = new TransportUnavailableError(
	...
	const readyPromise = new Promise<void>((resolve, reject) => {
		...
		const connectSource = () => {
			...
			const localSource: EventSourceLike = new EventSourceCtor(
				endpointCandidate,
			);
			source = localSource;
				...
				try {
					...
					if (event.type === "message") {
						forwardFrame({
							...
							clientMessageId:
								typeof (parsed as { clientMessageId?: unknown })
			...
			["message", "typing"].forEach((eventName) => {
				...
				sourceCleanup.push(() =>
					localSource.removeEventListener(
						eventName,
						handleMessageEvent as never,
					),
				);
			});
		};
			...
			try {
				...
				if (
					...
					forwardFrame({
						type: "message:ack",
						conversationId:
							normalizedMessage?.conversationId ?? parsed.conversationId,
						clientMessageId: parsed.clientMessageId ?? null,
...
const createWebSocketDriver = (
	...
	async start(startOptions) {
		...
		const factory =
			...
			((options) => defaultCreateWebSocket(options));

		return factory({ ...startOptions, endpoint, dependencies });
	},
});

const createPushDriver = (
	dependencies: TransportDependencies,
): TransportDriver => ({
	async start(startOptions) {
		const endpoint =
		...
		const factory =
			dependencies.createPush ?? ((options) => defaultCreatePush(options));
		...
		try {
			...
			if (dependencies.createWebTransport || endpoint) {
				if (
					...
					console.debug(
						"[transport:progressive] WebRTC failed, falling back to WebTransport",
						{
				...
				const webTransportFactory =
					dependencies.createWebTransport ??
					((options) => defaultCreateWebTransport(options));
...
const udpTransport =
	(dependencies: TransportDependencies): TransportFactory =>
	() =>
...
export function initializeMessagingTransport(
	options: {
