  ...
  new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(UNREAD_STORE_NAME)) {
        const store = db.createObjectStore(UNREAD_STORE_NAME, { keyPath: "id" });
        store.createIndex(UNREAD_CONVERSATION_INDEX, "conversationId", { unique: false });
        store.createIndex(UNREAD_DELIVERED_INDEX, "delivered", { unique: false });
      } else {
    ...
    request.onerror = () => {
      reject(request.error ?? new Error("Failed to open unread message storage"));
    };
  });
...
const runStoreRequest = (db, mode, executor) =>
  new Promise((resolve, reject) => {
    ...
    tx.onabort = () => reject(tx.error ?? new Error("Unread message transaction aborted"));
...
const handlePeerMessageEvent = async (payload) => {
  ...
  const recordId = messageId ? `${conversationId}::${messageId}` : `${conversationId}::${Date.now()}`;
  ...
  const record = {
    ...
    conversationTitle:
      typeof payload.conversationTitle === "string" ? payload.conversationTitle : null,
