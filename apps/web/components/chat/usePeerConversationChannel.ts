...
export function usePeerConversationChannel(options: {
	...
	const storagePromiseRef = useRef<Promise<ConversationStorage | null> | null>(
		null,
	);
	...
	const updateMessages = useCallback(
		(conversationId: string, incoming: ChatMessage[]): ConversationSnapshot => {
	...
	const notify = useCallback((conversationId: string, event: ChannelEvent) => {
		const listeners = listenersRef.current.get(conversationId);
		if (!listeners) return;
		listeners.forEach((listener) => {
			try {
	...
	useEffect(() => {
		...
		const handleMessage = (event: MessageEvent) => {
			...
			if (payload.type === "peer:queued-messages") {
				...
				) as QueuedMessageRecord[] | readonly QueuedMessageRecord[];
				const ackIds: string[] = [];
	...
	const resolvePending = useCallback(
		<T>(map: PendingMap<T>, key: string | null | undefined, value: T) => {
			if (!key) return false;
	...
	const rejectPending = useCallback(
		<T>(map: PendingMap<T>, key: string | null | undefined, error: Error) => {
			if (!key) return false;
			const entry = map.get(key);
			if (!entry) return false;
			map.delete(key);
		...
		(frame: PeerTransportIncomingFrame | null) => {
			...
			switch (frame.type) {
				...
				case "message": {
					...
					void publishServiceWorkerEvent(
						conversationId,
						updatedSnapshot,
						message,
						clientMessageId,
				...
				case "message:ack": {
					...
					if (!message) {
						...
						if (resolved) {
							resolvePending(pendingAcksRef.current, clientMessageId, resolved);
			...
			const handleEvent = (eventType: string, rawData: string | null) => {
				...
				if (eventType === "typing") {
					try {
						...
						console.error(
							"Failed to process HTTP fallback typing event",
							error,
						);
					}
				}
	...
	const syncViaHttp = useCallback(
		async ({ friendId, limit, signal }: SyncHistoryOptions) => {
			...
			if (!response.ok) {
				throw new Error(
					`HTTP history sync failed with status ${response.status}`,
				);
			}
	...
	const sendPresence = useCallback(async (presence: PeerPresenceUpdate) => {
	...
	const loadMore = useCallback(
		async ({ conversationId, cursor, limit, signal }: LoadMoreOptions) => {
			...
			return new Promise<LoadMoreResult>((resolve, reject) => {
				...
				transport.send(JSON.stringify(payload)).catch((error) => {
					cleanup();
	...
	const syncHistory = useCallback(
		async ({
			...
			const candidateConversationId =
				conversationIdHint ?? hydratedConversationId ?? null;
