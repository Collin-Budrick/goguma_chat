...
function createStorage(db: IDBDatabase): ConversationStorage {
	const run = <T>(
		...
		new Promise<T>((resolve, reject) => {
			...
			request.onerror = () => {
				reject(
					request.error ?? new Error("Conversation storage request failed"),
				);
			};

			tx.onabort = () => {
				reject(
					tx.error ?? new Error("Conversation storage transaction aborted"),
				);
			};
		});
	...
	return {
		async read(conversationId) {
			const result = await run("readonly", (store) =>
