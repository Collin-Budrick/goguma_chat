	...
	for (const message of messages) {
		if (
			...
			confirmedMessages.some((confirmed) =>
				isConfirmedMatch(message, confirmed),
			)
		) {
			continue;
		}
...
export default function ChatThread({
	...
	useEffect(() => {
		...
		const hydrateWithInitial =
			shouldUseInitialData && initialHydratedRef.current;
	...
	useEffect(() => {
			...
			if (event.type === "presence") {
				...
				if (event.presence.kind === "read") {
					if (event.presence.userId !== viewerId) {
						const profile = getParticipantProfile(
							conversation,
							event.presence.userId,
						);
						const name = profile
							? getContactName(profile)
							: t("thread.presence.readFallback");
						setPresenceToast(
				...
				if (event.presence.kind === "delivery") {
					if (event.presence.userId !== viewerId) {
						...
						const name = profile
							? getContactName(profile)
							: t("thread.presence.deliveryFallback");
						setPresenceToast(
		...
		return unsubscribe;
	}, [conversation, subscribeMessages, t, viewerId, playPresenceChime]);
	...
	const handleClearHistory = useCallback(() => {
		...
		if (
			...
			!window.confirm(t("thread.settings.local.options.clearHistory.confirm"))
		) {
			return;
		}
												...
												<div className="mt-3 flex flex-col gap-2">
													...
													>
														<span className={syncButtonLabelClasses}>
															{isSyncingHistory
																? t(
																		"thread.settings.local.options.syncHistory.syncing",
																	)
																: t(
																		"thread.settings.local.options.syncHistory.label",
																	)}
														</span>
														<span className={syncButtonDescriptionClasses}>
															{t(
																"thread.settings.local.options.syncHistory.description",
															)}
														</span>
													</button>
													<button
													...
													>
														<span className={clearButtonLabelClasses}>
															{t(
																"thread.settings.local.options.clearHistory.label",
															)}
														</span>
														<span className={clearButtonDescriptionClasses}>
															{t(
																"thread.settings.local.options.clearHistory.description",
															)}
														</span>
													</button>
												</div>
