...
function ensureProfile(profile: FriendRequestSummary["sender"]): FriendRequestSummary["sender"] {
  return {
    id: profile.id,
...
function normalizeRequest(request: Partial<FriendRequestSummary>): FriendRequestSummary {
  const createdAt = toISOString(request.createdAt);
  const updatedAt = request.updatedAt ? toISOString(request.updatedAt) : null;
  const respondedAt = request.respondedAt ? toISOString(request.respondedAt) : null;
...
function normalizeState(value: Partial<ContactsState> | null | undefined): ContactsState {
...
function reducer(state: ContactsState, action: ContactsAction): ContactsState {
  switch (action.type) {
    ...
    case "removeFriend": {
      return {
        ...state,
        friends: state.friends.filter((friend) => friend.friendshipId !== action.friendshipId),
      };
    }
    case "addIncoming": {
      const next = state.incoming.filter((request) => request.id !== action.payload.id);
      return {
        ...state,
    ...
    case "removeIncoming": {
      return {
        ...state,
        incoming: state.incoming.filter((request) => request.id !== action.requestId),
      };
    }
    case "addOutgoing": {
      const next = state.outgoing.filter((request) => request.id !== action.payload.id);
      return {
        ...state,
    ...
    case "removeOutgoing": {
      return {
        ...state,
        outgoing: state.outgoing.filter((request) => request.id !== action.requestId),
      };
    }
    default:
      return state;
...
export default function ContactsClient({ cacheKey, viewerId, initialState }: ContactsClientProps) {
      ...
      startTransition(async () => {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(errorPayload.error ?? "Unable to create friend request");
          }
    ...
    startTransition(async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorPayload.error ?? "Unable to accept friend request");
        }
        ...
        dispatch({ type: "removeFriend", friendshipId: optimisticFriend.friendshipId });
        handleError(error instanceof Error ? error.message : t("errors.request"));
      } finally {
  ...
  const declineRequest = (request: FriendRequestSummary) => {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(`/api/friend-requests/${request.id}/decline`, {
          method: "POST",
        });
        ...
        if (!response.ok) {
          ...
          throw new Error(errorPayload.error ?? "Unable to decline friend request");
        }
  ...
  const cancelRequest = (request: FriendRequestSummary) => {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(`/api/friend-requests/${request.id}/cancel`, {
          method: "POST",
        });
        ...
        if (!response.ok) {
          ...
          throw new Error(errorPayload.error ?? "Unable to cancel friend request");
        }
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr,1fr] xl:grid-cols-[3fr,1.2fr]">
        <section className="space-y-6">
          <FriendList
            ...
            countLabel={t("lists.friends.count", { count: state.friends.length })}
