...
async function DashboardContent({ params }: PageProps) {
	...
	return (
		<div className="mx-auto flex w-full max-w-5xl flex-col gap-10 px-4 py-8 sm:px-6 lg:px-0">
						...
						{QUEUE_KEYS.map((key) => (
							...
							>
								<span className="text-white">
									{t(`queues.items.${key}.name`)}
								</span>
								<span>
									<span className="text-white/80">
										{t(`queues.items.${key}.count`)}
									</span>{" "}
									<span className="text-white/50">
										{t(`queues.items.${key}.status`)}
									</span>
								</span>
							</li>
						))}
