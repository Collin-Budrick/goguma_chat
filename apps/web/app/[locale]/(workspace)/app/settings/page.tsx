...
export default function SettingsPage() {
	...
	const cardTone = isLightThemeEnabled
		? {
				active: {
					container: "border-black bg-black text-white",
					description: "text-white/70",
				},
				inactive: {
					container: "border-black/10 bg-white text-slate-900",
					description: "text-slate-500",
				},
			}
		: {
				active: {
					container: "border-white/40 bg-white text-black",
					description: "text-black/60",
				},
				inactive: {
					container: "border-white/10 bg-black text-white",
							...
							>
								<div className="flex items-center justify-between text-sm font-medium">
									...
									<span className="text-xs uppercase tracking-[0.3em]">
										{entry.active
											? t("preferences.state.on")
											: t("preferences.state.off")}
									</span>
								</div>
								<p className={`mt-2 text-xs ${tone.description}`}>
									{entry.description}
								</p>
							</button>
						);
