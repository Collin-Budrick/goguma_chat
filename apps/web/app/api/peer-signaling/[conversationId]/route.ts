...
const logPeerSignalingServer = (
	message: string,
	meta?: Record<string, unknown>,
) => {
	const payload = meta ? JSON.stringify(meta) : "";
	console.info(
		`[peer-signaling:server] ${message}${payload ? ` ${payload}` : ""}`,
	);
};
...
export async function POST(
	...
	if (
		(role === "host" && kind !== "offer") ||
		(role === "guest" && kind !== "answer")
	) {
		return NextResponse.json(
			{ error: "Unexpected token kind for role" },
			{ status: 400 },
		);
	}
...
export async function GET(
	...
	if (mode === "poll") {
		const tokens = consumePendingForRole(conversationId, role).map(
			formatTokenPayload,
		);
